<?php 
	...
	if(!empty($_FILES["fileToUploadParecerAcesso"]['name'])){
		// For this, I would generate a unqiue random string for the key name. But you can do whatever.
		$keyName = 'daniel/' .$dataLocal. basename($_FILES["fileToUploadParecerAcesso"]['name']);
		$pathInS3 = 'https://s3.us-east-1.amazonaws.com/' . $bucketName . '/' . $keyName;

		// Add it to S3
		try {
			// Uploaded:
			$file = $_FILES["fileToUploadParecerAcesso"]['tmp_name']; //NOME TEMP DO ARQUIVO NO SERVIDOR
	...
	$aprovarDocumentosProjeto = $pdo->prepare("UPDATE projetos SET status_projeto = 'finalizado', img_parecer_projeto = '$linkImg' WHERE id_projeto = '$idProjeto'");
	$aprovarDocumentosProjeto->execute();

	$tipoNotificacao = "finalizado";
