<?php 
	...
	$projetoBuscado = $_POST['pesquisar-projetos'];

	echo $projetoBuscado;

	$verificaUsuarios = $pdo->prepare("SELECT * FROM usuarios INNER JOIN projetos ON usuarios.id_usuario = projetos.id_usuario WHERE nome_projeto LIKE '%$projetoBuscado%' GROUP BY usuarios.id_usuario ORDER BY COUNT(projetos.id_projeto)  DESC");
...
<?php if($_SESSION['tipo-usuario'] == 'master'){?>
	...
	<section class="nav-painel separador">
		...
		<ul>		
			<li>Projetos</li>
			<li><h3>Total de projetos: <?php echo count($totalProjetos) ?></h3></li>
		</ul>
	...
	<section class="projetos-wrapper separador ">
		...
		<div class="lista-projetos-wrapper">
			<div class="lista-projetos" style="display: block" id="lista-projetos<?php echo $totalUsuarios[$i]['id_usuario']?>">
				<table>
					<tr>
						<th>ID:</th>
						<th>Nome:</th>
					...
					<?php 
						...
						$verificaProjetosPorUsuario = $pdo->prepare("SELECT * FROM projetos WHERE id_usuario = $idUsuario AND nome_projeto LIKE '%$projetoBuscado%'");
					...
					<tr>
						<td><?php echo $totalProjetosPorUsuario[$j]['id_projeto']?></td>
